...
本文档提供 Django 适配虚谷数据库(XuguDB)的完整配置流程。Django 是 Python 生态中最流行的 Web 框架之一，通过虚谷数据库专用的方言包 `xgcondb`，可以无缝集成 Django 框架的各种功能，包括 ORM、迁移、管理后台等。

## 前置条件

1. 已安装 Python 3.x 环境
2. 已部署虚谷数据库 v12.0.0 或更高版本
3. 已获取虚谷数据库 Python 驱动和 Django 方言包
...
| 字段类型 | 说明 | 示例 |
|----------|------|------|
| CharField | 字符串字段 | `models.CharField(max_length=100)` |
| TextField | 文本字段 | `models.TextField()` |
| IntegerField | 整数字段 | `models.IntegerField()` |
| FloatField | 浮点字段 | `models.FloatField()` |
| DecimalField | 十进制字段 | `models.DecimalField(max_digits=10, decimal_places=2)` |
| BooleanField | 布尔字段 | `models.BooleanField(default=True)` |
| DateField | 日期字段 | `models.DateField()` |
| DateTimeField | 日期时间字段 | `models.DateTimeField()` |
| EmailField | 邮箱字段 | `models.EmailField()` |
| URLField | URL字段 | `models.URLField()` |
| FileField | 文件字段 | `models.FileField(upload_to='uploads/')` |
| ImageField | 图片字段 | `models.ImageField(upload_to='images/')` |
